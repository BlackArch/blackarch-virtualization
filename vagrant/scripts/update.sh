...
{
	MD5SUM=$(curl --silent http://mirror.rackspace.com/archlinux/iso/${YEAR}.${MONTH}.01/md5sums.txt | grep archlinux-${YEAR}.${MONTH}.01-x86_64.iso | awk ' { print $1 } ')
...
{
	if curl -s --output /dev/null --silent --head --fail "http://mirror.rackspace.com/archlinux/iso/${YEAR}.${MONTH}.01/archlinux-${YEAR}.${MONTH}.01-x86_64.iso"; then
		echo "updating URL"
